    ...
    <section className={`panel impl ${complete ? 'is-complete' : 'is-incomplete'}`}>
      <div className="panel-head">
        <div>
          ...
          <span className="panel-meta">
            <span className={`mode-chip ${mode}`}>{mode}</span> mode · {implemented}/{total}{' '}
            functions live
          </span>
        </div>
      ...
      {mode === 'student' && !complete && (
        <p className="impl-hint">
          Running in <strong>student</strong> mode, but {total - implemented} function
          {total - implemented > 1 ? 's are' : ' is'} still missing — the app is falling back to the
          reference implementation for those. Export them from the notebook (Parts&nbsp;1.4 / 2.6)
          into <code>solutions/</code> to go fully live.
        </p>
      )}
    </section>
  )
